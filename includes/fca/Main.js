...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
    ...
    try {
        if (appState) {
            ...
            try {
                // Force encryption off to prevent UTF-8 errors
                const encryptFeature = false; 
                switch (encryptFeature) {
                    case true: {
    ...
    mainPromise = mainPromise
        ...
        .then(function(res) {
            ...
            if (!Obj || !Obj.ctx || !Obj.api) {
                throw { error: "Login failed - Your appstate/cookie is invalid or expired. Please get a new one by logging into Facebook in an incognito browser and re-exporting your cookies." };
            }
            ctx = Obj.ctx;
            api = Obj.api;
            return res;
        ...
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.FastConfig.ResetDataLogin) {
                        ...
                        global.Fca.Require.fs.writeFileSync(process.cwd() + '/Razafca.json', JSON.stringify(global.Fca.Require.FastConfig, null, 4));
                    }
