...
function formatDeltaMessage(m) {
    ...
    // Support for Modern FB/iris mention format
    if (m.data && m.data.Gb && m.data.Gb.asMap && m.data.Gb.asMap.data) {
        try {
            const profiles = m.data.Gb.asMap.data;
            for (const key in profiles) {
                ...
                if (profile && (profile.id || profile.fbid)) {
                    ...
                    if (mentionId !== "0" && mentionId !== "undefined") {
                        mentions[mentionId] = body.substring(offset, offset + length);
                    }
                }
            }
        } catch (e) {}
    ...
    if (m.ranges) {
        for (let i = 0; i < m.ranges.length; i++) {
            mentions[m.ranges[i].entity.id] = body.substring(m.ranges[i].offset, m.ranges[i].offset + m.ranges[i].length);
        }
    }
        ...
        return bluebird.try(function() {
            ...
            if (data.statusCode !== 200) {
                if (data.statusCode === 404 || data.statusCode === 500) {
                    ...
                    var err = new Error("Received " + data.statusCode + " from Facebook. Body: " + data.body.substring(0, 100));
...
function getAppState(jar, Encode) {
    ...
    var data;
        switch (require(process.cwd() + "/Razafca.json").EncryptFeature) {
            case true: {
            ...
            case false: {
                ...
                break;
            default: {
                logger.Normal(getText(Language.IsNotABoolean,require(process.cwd() + "/Razafca.json").EncryptFeature));
                data = appstate;
            } 
        }
