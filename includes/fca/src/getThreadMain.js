...
function formatThreadGraphQLResponse(data) {
  try {
    ...
    if (!messageThread || !messageThread.thread_key) {
      ...
      throw new Error("Invalid message thread data");
    }
  } catch (err) {
  ...
  return function getThreadInfoGraphQL(threadID, callback) {
    ...
    defaultFuncs
      ...
      resData.map(function (x,y) {
        var threadData = x["o"+y] ? x["o"+y].data : null;
        if (threadData) {
          ThreadInfo.push(formatThreadGraphQLResponse(threadData));
        } else {
          log.warn("getThreadInfoGraphQL", "Missing data for index " + y);
        }
      });
