  ...
  function sendContent(form, threadID, isSingleUser, messageAndOTID, callback) {
    ...
    defaultFuncs
      ...
      .then(function (resData) {
        ...
        if (!resData.payload || !resData.payload.actions || !Array.isArray(resData.payload.actions) || resData.payload.actions.length === 0) {
          return callback(null, { threadID: threadID, messageID: null, timestamp: Date.now() });
        }
...
//Full Fix sendMessage - Updated for 16 digit groups + Admin inbox support
  if (utils.getType(threadID) === "Array") sendContent(form, threadID, false, messageAndOTID, callback);
    else {
      var THREADFIX = "ThreadID".replace("ThreadID",threadID);
        
        // Check if this is admin ID (for notifications) - force single user chat
        var isAdminID = false;
        if (global.config && global.config.ADMINBOT) {
          isAdminID = global.config.ADMINBOT.includes(threadID) || (global.config.NDH && global.config.NDH.includes(threadID));
        }
        
        if (isAdminID) {
          // Admin notifications - always single user
        ...
        // User IDs are typically 14-15 digits, Groups are 15-16 digits
        else if (THREADFIX.length <= 14 || global.Fca.isUser.includes(threadID)) {
          // Single user chat
          sendContent(form, threadID, !isGroup, messageAndOTID, callback);
        }
        else if (THREADFIX.length >= 15 || global.Fca.isThread.includes(threadID)) {
          // Group chat (15 or 16 digits)
  ...
  function handleMention(msg, form, callback, cb) {
    if (msg.mentions) {
      for (let i = 0; i < msg.mentions.length; i++) {
        ...
        // Add additional mention tags for newer FCA compatibility
        form["profile_xmd[" + i + "][type]"] = "p";
        form["profile_xmd[" + i + "][label]"] = tag;
      }
    }
    cb();
  }
