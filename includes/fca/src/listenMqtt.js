    ...
    global.mqttClient.on('connect', () => {
        ...
        if (process.env.OnStatus === undefined) {
            ...
            const SettingMemoryManager = {
                ...
                interval: 60 * 1000,
                logLevel: 'warn',
                logFile: path.join(process.cwd(), 'Raza_DataBase', 'memory.log'),
...
function parseDelta(defaultFuncs, api, ctx, globalCallback, {
  ...
  if (delta.class === 'NewMessage') {
    ...
    // Fix: Extract mentions from delta.messageMetadata.data.data.Gb (Iris format)
    if (!delta.data) delta.data = {};
    const irisData = delta.messageMetadata?.data?.data;
    if (irisData && irisData.Gb && irisData.Gb.asMap && irisData.Gb.asMap.data) {
        delta.data.Gb = irisData.Gb;
    }

    if (delta.data && delta.data.Gb && delta.data.Gb.asMap && delta.data.Gb.asMap.data) {
        try {
            const profiles = delta.data.Gb.asMap.data;
            delta.mentions = delta.mentions || {};
            for (const key in profiles) {
                ...
                if (profile && (profile.id || profile.fbid)) {
                    ...
                    if (mentionId !== "0" && mentionId !== "undefined") {
                        delta.mentions[mentionId] = (delta.body || "").substring(offset, offset + length);
                    }
                }
            }
        } catch (e) {}
    } else if (delta.data && delta.data.prng) {
        try {
            const mdata = JSON.parse(delta.data.prng);
            delta.mentions = delta.mentions || {};
            for (let i = 0; i < mdata.length; i++) {
                delta.mentions[mdata[i].i] = (delta.body || "").substring(mdata[i].o, mdata[i].o + mdata[i].l);
            }
        } catch (e) {}
    }
    // Extra fix for iris/modern format in NewMessage
    if (delta.ranges) {
        delta.mentions = delta.mentions || {};
        for (let i = 0; i < delta.ranges.length; i++) {
            delta.mentions[delta.ranges[i].entity.id] = (delta.body || "").substring(delta.ranges[i].offset, delta.ranges[i].offset + delta.ranges[i].length);
        }
    }
    ...
    const resolveAttachmentUrl = (i) => {
      if (!delta.attachments || i === delta.attachments.length || utils.getType(delta.attachments) !== 'Array') {
        ...
        try {
          ...
          if (delta.mentions) fmtMsg.mentions = delta.mentions;
        } catch (err) {
    ...
    if (clientPayload && clientPayload.deltas) {
      for (const delta of clientPayload.deltas) {
        if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
          ...
          // Fix: Extract mentions for message_reply (Iris format)
          if (!delta.deltaMessageReply.message.data) delta.deltaMessageReply.message.data = {};
          const replyIrisData = delta.deltaMessageReply.message.messageMetadata?.data?.data || delta.deltaMessageReply.repliedToMessage?.messageMetadata?.data?.data;
          if (replyIrisData && replyIrisData.Gb && replyIrisData.Gb.asMap && replyIrisData.Gb.asMap.data) {
            delta.deltaMessageReply.message.data.Gb = replyIrisData.Gb;
          }

          // Fix: Extract mentions for message_reply
          let mentions = {};
          ...
          } else if (delta.deltaMessageReply.message && delta.deltaMessageReply.message.data && delta.deltaMessageReply.message.data.prng) {
            try {
              const mdata_reply = JSON.parse(delta.deltaMessageReply.message.data.prng);
              for (let i = 0; i < mdata_reply.length; i++) {
                mentions[mdata_reply[i].i] = (delta.deltaMessageReply.message.body || "").substring(mdata_reply[i].o, mdata_reply[i].o + mdata_reply[i].l);
              }
            } catch (e) {}
          }
          // Extra fix for iris/modern format in replies
          if (delta.deltaMessageReply.message && (delta.deltaMessageReply.message.ranges || (delta.deltaMessageReply.repliedToMessage && delta.deltaMessageReply.repliedToMessage.ranges))) {
            const ranges = delta.deltaMessageReply.message.ranges || delta.deltaMessageReply.repliedToMessage.ranges;
            for (let i = 0; i < ranges.length; i++) {
              const range = ranges[i];
              mentions[range.entity.id] = (delta.deltaMessageReply.message.body || "").substring(range.offset, range.offset + range.length);
            }
          }
          if (delta.deltaMessageReply.message) {
            delta.deltaMessageReply.message.mentions = mentions;
          }
          ...
          // Merge extracted mentions with existing ones if any
          if (m_id.length > 0) {
