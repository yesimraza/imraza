...
try {
    ...
    console.log('Note: deasync not available (only needed for Replit environment)');
}

if (!fs.existsSync(process.cwd() + '/Raza_DataBase')) {
    fs.mkdirSync(process.cwd() + '/Raza_DataBase');
    fs.writeFileSync(process.cwd() + '/Raza_DataBase/A_README.md', 'This folder is used by ChernobyL(NANI =)) ) to store data. Do not delete this folder or any of the files in it.', 'utf8');
}
var db = new BetterDB(process.cwd() + "/Raza_DataBase/SyntheticDatabase.sqlite");
