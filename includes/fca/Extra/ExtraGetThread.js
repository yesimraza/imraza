...
const fs = require("fs");
language = language.find(i => i.Language == require(process.cwd() + "/Razafca.json").Language).Folder.ExtraGetThread;
const Always_True = [];
if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "json") {
    if (!fs.existsSync(process.cwd() + "/Raza_DataBase/Threads.json")) {
        fs.writeFileSync(process.cwd() + "/Raza_DataBase/Threads.json",JSON.stringify({}));
    }
}
    ...
    logger.Warning("Database_Type in Razafca.json is not valid. Only default and json are valid.");
    process.exit(0);
}
...
exports.createData = function(threadID,threadData) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            try {
                var data = require(process.cwd() + "/Raza_DataBase/Threads.json");
            }
            catch (e) {
                var data = {};
                fs.writeFileSync(process.cwd() + "/Raza_DataBase/Threads.json",JSON.stringify(data));
            }
...
exports.deleteAll = function(data) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            var data1 = require(process.cwd() + "/Raza_DataBase/Threads.json");
            for (let i of data) {
                delete data1[String(i)];
            }
            fs.writeFileSync(process.cwd() + "/Raza_DataBase/Threads.json",JSON.stringify(data1));
        }
        catch (e) {
            console.log(e);
        }
