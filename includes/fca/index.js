    ...
    Data: new Object({
        ObjFastConfig: {
            ...
            "AntiStuckAndMemoryLeak": {
                ...
                "LogFile": {
                    "Use": false,
                    "Explain": "Record memory usage logs to fix errors. Default location: Raza_DataBase/memory.logs"
                }
            }
        },
        CountTime: function() {
...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/Razafca.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/Razafca.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/Razafca.json");
}
catch (e) {
    ...
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/Razafca.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/Razafca.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
