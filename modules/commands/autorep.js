...
module.exports.run = function({ api, event, args }) {
	...
	if (content.indexOf(`del`) == 0) {
		...
		if (!delThis) return api.sendMessage("Not found autorep bạn cần xóa", threadID, messageID);
		return readFile(__dirname + "/cache/data/autorep.json", "utf-8", (err, data) => {
			...
			if (!getThread.some(item => item.in == delThis)) return api.sendMessage("Not found autorep bạn cần xóa", threadID, messageID);
