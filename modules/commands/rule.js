...
module.exports.run = ({ event, api, args, permssion }) => {
    ...
    switch (args[0]) {
        ...
        case"all": {
            ...
            if (msg.length == 0) return api.sendMessage("⚠️ Groups của bạn hiện tại chưa có danh sách luật để hiển thị!", threadID, messageID);
        ...
        case "del": {
            if (!isNaN(content) && content > 0) {
                ...
                if (thisThread.listRule.length == 0) return api.sendMessage("⚠️ Groups của bạn chưa có danh sách luật để có thể xóa!", threadID, messageID);
                thisThread.listRule.splice(content - 1, 1);
                api.sendMessage(`✅ Deleted successfully luật có số thứ tự thứ ${content}`, threadID, messageID);
                break;
            }
            else if (content == "all") {
                if (permssion == 0) return api.sendMessage("❎ Bạn không đủ quyền hạn để có thể sử dụng xóa luật!", threadID, messageID);
                if (thisThread.listRule.length == 0) return api.sendMessage("⚠️ Groups của bạn chưa có danh sách luật để có thể xóa!", threadID, messageID);
                thisThread.listRule = [];
                api.sendMessage(`✅ Deleted successfully toàn bộ luật của nhóm!`, threadID, messageID);
                break;
            }
        }
        default: {
